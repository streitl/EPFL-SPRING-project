...
def find_adversarial_examples(srr_model, X, y, can_change):
    ...
    adversarial examples that have changed the model prediction label, and that are only one
    feature value away from a real data point.
    ...
    ## Construct a list of potential adversarial examples by deforming
    ## exactly one feature at a time from each correctly classified point
    ...
    for index, data in tqdm(correctly_classified.iterrows(), total=correctly_classified.shape[0]):
        ...
        # Go through features that can be changed
        for feat in modifiable_features:
            
            # Change feature for each of the possible values except original
            for category in feat_to_cats[feat]:
                if category == data[feat]:
                    continue
                
                # Modify the original data point and add it to the list
                deformed = data.copy()
                deformed[feat] = category
