...
class SRR(BaseEstimator, ClassifierMixin):
    ...
    @staticmethod
    def new_model_with_same_params_as(model):
        """
    ...
    def fit(self, X, y, kind="linear", verbose=0):
        ...
        # Constructing the model dataframe with the weights
        ...
        # This is not strictly needed but helps understanding the model
        self.df['relative'] = self.df['original'] / w_max
    ...
    def get_weight(self, feature, category, column='M'):
        ...
        try:
            ...
            raise ValueError(f'The model does not have weights for {feature}, {category}, {column}')
    ...
    def __contains__(self, item):
        ...
        if len(item) == 1: # item is just the feature
            assert item
